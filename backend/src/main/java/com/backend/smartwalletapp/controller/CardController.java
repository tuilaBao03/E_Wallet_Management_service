...
public class CardController {
    ...
    @PostMapping()
    @PreAuthorize("hasRole(Roles.USER.name()) or hasRole(Roles.ADMIN.name())")
    public ApiResponse<CreateCardReponse> createCard(
        @RequestBody @Valid CreateCardRequest request){
        return ApiResponse.<CreateCardReponse>builder()
        .result(cardService.createCardV3(request))
        .code(cardService.createCardV3(request).getRetCode())
        .message(cardService.createCardV3(request).getRetMsg())
        .build();
    }
