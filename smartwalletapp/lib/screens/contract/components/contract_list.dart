...
class ContractList extends StatefulWidget {
  ...
  final Function(Contract) onDetailSelected;
  final User user;
  ...
  const ContractList({
    ...
    required this.title,
    required this.onDetailSelected, required this.user, required this.currentPage,
  });
...
class _ContractListState extends State<ContractList> {
  ...
  void _filterByDate() {
    setState(() {
      _filteredTrans = widget.object.where((contract) {
        if (_startDate == null || _endDate == null) return true;
        return contract.createdDate.isAfter(_startDate!.subtract(Duration(days: 1))) &&
            contract.createdDate.isBefore(_endDate!.add(Duration(days: 1)));
      }).toList();
    });
  }
...
DataRow recentFileDataRow(Contract fileInfo, VoidCallback onDetail,bool currentPage) {
  return DataRow(
    cells: [
      
      DataCell(Text(fileInfo.contractID)),
      DataCell(Text(DateFormat('dd/MM/yyyy').format(fileInfo.createdDate))),
      DataCell(
        IconButton(
